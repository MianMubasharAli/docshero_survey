                              ...
                              provider.surveyModel!.steps![provider.questionsIndex].value!.questions![provider.chaptersQuestionsIndex].configuration2!.conditionsForProductSelection?.forEach((element3) {
                                ...
                                element3.options?.forEach((element4) {
                                  ...
                                  element3.options?.forEach((option) {
                                    checkIndexForConditionString++;
                                    bool abc= option.condition == "contains" ?
                                    eval("(\"${provider.surveyModel!.steps![provider.questionsIndex].value!.questions![provider.chaptersQuestionsIndex].configuration2!.options?[widget.index].value.toString()}\""
                                        "${".contains(\"${option.value}\")"} )") :
                                    eval("(\"!${provider.surveyModel!.steps![provider.questionsIndex].value!.questions![provider.chaptersQuestionsIndex].configuration2!.options?[widget.index].value.toString()}\""
                                        "${".contains(\"${option.value}\")"} )");
                                    conditionList.add("$abc ${ checkIndexForConditionString < element3.options!.length ? element3.options![checkIndexForConditionString].optionOperator : ""}  ");

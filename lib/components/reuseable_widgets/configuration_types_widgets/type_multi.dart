  ...
  Widget build(BuildContext context) {
    ...
    return  Container(
        ...
        child: CheckboxListTile(
          ...
          provider.surveyModel?.steps?[provider.questionsIndex].value?.questions?[provider.chaptersQuestionsIndex].value?.configuration?.options?[widget.index].isSelected
          ,
          ...
          Text("${provider.surveyModel?.steps?[provider.questionsIndex].value?.questions?[provider.chaptersQuestionsIndex].value?.configuration?.options?[widget.index].title}",style: TextStyle(fontSize: size.width * 0.05)),
          onChanged: (onChanged) async{
            ...
            //             bool value=false;
            //             // provider.surveyModel!.steps![provider.questionsIndex].value!.configuration!.options?.forEach((aboveOption) {
            //             //   if(aboveOption.id==element4.option?.id){
            //             //
            //             //   }
            //             // });
            //             String? condition="";
            ...
            //     provider.surveyModel?.steps?[provider.questionsIndex].value?.questions?[provider.chaptersQuestionsIndex].configuration?.options?[widget.index].isSelected=onChanged!;
            //     if(onChanged==false){
            //       provider.productList.forEach((element) { //cart products
            //         //selected option
            //         element.belongsTo?.forEach((element2) {
            ...
            //                 checkIndexForConditionString++;
            //                 bool abc=false;
            //                 provider.surveyModel!.steps![provider.questionsIndex].value!.questions![provider.chaptersQuestionsIndex].configuration!.options?.forEach((aboveOption) {
            //                   if(aboveOption.id==option.option?.id){
            //                     abc= option.condition == "checked" ? aboveOption.isSelected == true ? true : false : aboveOption.isSelected == false ? true : false;
            //                   }
            //                 });
            ...
            //               return element2==element3;
            //             });
            //             provider.surveyModel!.steps![provider.questionsIndex].value!.questions![provider.chaptersQuestionsIndex].configuration!.conditionsForProductSelection?.forEach((conditionForProductSelection) {
            //               double discount=double.parse(conditionForProductSelection.discount.toString());
            //               if(conditionForProductSelection.id == element2){
            //                 conditionForProductSelection.products?.forEach((product) {
            //                   if(provider.productList[element].id == product.id){
            ...
            if(provider.surveyModel?.steps?[provider.questionsIndex].type == "question"){
              provider.surveyModel?.steps?[provider.questionsIndex].value?.configuration?.options?[widget.index].isSelected=onChanged!;
             await provider.optionMultiple(
                  onChanged!,
                  provider.surveyModel!.steps![provider.questionsIndex].value!.configuration!,
                  widget.index);
            }else{
              provider.surveyModel?.steps?[provider.questionsIndex].value?.questions?[provider.chaptersQuestionsIndex].value?.configuration?.options?[widget.index].isSelected=onChanged!;
              await provider.optionMultiple(
                  onChanged!,
